...
export default function TrussType1Generator(
  length: number,
  height: number,
  load: number,
  spans: number = 6
  ...
  function generateBracing() {
    ...
    for (let index = 0; index < halfSpan; index++) {
      const x0 = index * spanLength;
      const x1 = (index + 1) * spanLength;
      const y0 = index % 2 === 0 ? -height / 2 : topChordLine.getY(x0);
      const y1 = index % 2 === 0 ? topChordLine.getY(x1) : -height / 2;
      const startPoint = new Point(x0, y0);
      const endPoint = new Point(x1, y1);
